...
func TestRecentRootNativeWriteLastWriteWins(t *testing.T) {
	...
	for i, root := range []common.Hash{root1, root2} {
		...
		if got := uint64(100_000) - remaining.RegularGas - remaining.StateGas; got != wantUsed {
...
func TestRecentRootNativeWriteRejectsInvalidCalls(t *testing.T) {
	...
	source := common.HexToAddress("0x1234")
	valid := make([]byte, 64)
	if _, _, err := evm.Call(source, params.RecentRootAddress, valid[:63], vm.NewGasBudget(100_000, 0), new(uint256.Int)); err == nil {
		t.Fatal("short calldata accepted")
	}
	if _, _, err := evm.Call(source, params.RecentRootAddress, valid, vm.NewGasBudget(100_000, 0), uint256.NewInt(1)); err == nil {
		t.Fatal("nonzero value accepted")
	}
	if _, _, err := evm.StaticCall(source, params.RecentRootAddress, valid, vm.NewGasBudget(100_000, 0)); err == nil {
		t.Fatal("static write accepted")
	}
	if _, _, err := evm.CallCode(source, params.RecentRootAddress, valid, vm.NewGasBudget(100_000, 0), new(uint256.Int)); err == nil {
		t.Fatal("CALLCODE write accepted")
	}
	if _, _, err := evm.DelegateCall(source, source, params.RecentRootAddress, valid, vm.NewGasBudget(100_000, 0), new(uint256.Int)); err == nil {
		t.Fatal("DELEGATECALL write accepted")
	}
	if _, _, err := evm.Call(source, params.RecentRootAddress, valid, vm.NewGasBudget(100, 0), new(uint256.Int)); err == nil {
